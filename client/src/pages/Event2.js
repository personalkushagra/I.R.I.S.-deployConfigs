...
const Event2 = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://${process.env.REACT_APP_BACKEND_URL}/api/event2`, formData);
  ...
  const checkoutHandler = async () => {
    try {
      const amount = 1;
      const { data: { key } } = await axios.get(`http://${process.env.REACT_APP_BACKEND_URL}/api/getkey`);
      const { data: { order } } = await axios.post(`http://${process.env.REACT_APP_BACKEND_URL}/api/checkout`, { amount });
      ...
      const options = {
        ...
        order_id: order.id,
        callback_url: `http://${process.env.REACT_APP_BACKEND_URL}/api/paymentverification?leader_email=${formData.leader_email}`,
        prefill: {
